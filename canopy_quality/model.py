...
class treenet:
    ...
    def __init__(self, model_weights: dict = None):
        ...
        if model_weights is None:
            ...
            if self.name == "ms":
                model_weights = dict(
                    url="https://zenodo.org/records/10149637/files/pytorch_mtloss_partshared_manual_allbands.pt",
        ...
        def defineUNetModel_partiallyshared():
            ...
            class UNet(nn.Module):
                ...
                def forward(self_unet, x):
                    ...
                    out_tree_height = self_unet.conv_lasta(
                        x1)  # looks like i don't need any additional activation here for linear
    ...
    def predict_batch(self, images: np.ndarray, batch_size: int):
        ...
        if (len(images.shape) != 4) or (images.shape[1] != 14 and images.shape[1] != 3) or (
                images.shape[2] != 240 or images.shape[3] != 240):
            raise ValueError("Images must be of shape (N, 14,240,240) or (N, 3,240,240)")

        if len(images) <= 1:
            raise ValueError(
                "You must have multiple images to use predict_batch. If you only have one image try the predict function.")
        ...
        if images.shape[1] != self.num_bands_input:
            raise ValueError(
                "Ensure the model you have selected and the images you input have matching numbers of light bands.")
        ...
        with torch.inference_mode():
            for i_batch, sample_batched in enumerate(my_dataloader):
                ...
                pred_tree_heights = torch.squeeze(pred_tree_heights) * torch.squeeze(
                    pred_tree_masks)  # 0s get rid of non tree pixels
    ...
    def predict(self, image: np.ndarray) -> np.ndarray:
        ...
        if image.shape != (14, 240, 240) and image.shape != (3, 240, 240):
            raise ValueError(
                "Image shape must be of (14,240,240) or (3,240,240). If you have multiple images, use predict_batch.")
        ...
        np_image = np.expand_dims(image[:self.num_bands_input], axis=0)
        np_image[
            np_image < .0000001] = 0  # Ensure no very small values (might have been an artifact of the original data)
        ...
        with torch.inference_mode():
            ...
            pred_tree_height = torch.squeeze(pred_tree_height) * torch.squeeze(
                pred_tree_mask)  # 0s get rid of non tree pixels
